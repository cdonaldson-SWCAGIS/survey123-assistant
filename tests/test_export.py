...
def test_get_survey_args():
    ...
    with tempfile.NamedTemporaryFile(suffix=".xlsx", delete=False) as tmp:
        ...
        survey = Survey(
            ...
            label="Test Survey",
            items=[
                Question(type=QuestionTypes.text, name="q1", label="Question 1"),
                QuestionGroup(
                    name="group1",
                    label="Group 1",
                    items=[
...
def test_items_to_dfs_nested_groups(complex_survey):
    ...
    nested_group = QuestionGroup(
        ...
        label="Outer Group",
        items=[
            Question(type=QuestionTypes.text, name="q1", label="Q1"),
            QuestionGroup(
                name="inner",
                label="Inner Group",
                items=[Question(type=QuestionTypes.text, name="q2", label="Q2")],
            ),
        ],
    )
